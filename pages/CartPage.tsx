...
export const CartPage: React.FC = () => {
  ...
  const handleQuantityChange = (item: CartItem, newQuantity: number) => {
    if (newQuantity <= 0) {
      removeFromCart(item.id); // Assuming item.id is the cartItemId for now
    } else {
      updateQuantity(item.id, Math.min(newQuantity, item.stock)); // Assuming item.id is the cartItemId
    }
  };
    ...
    <div className="py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white shadow-lg rounded-lg">
          {cartItems.map(item => (
            <div key={item.id} className="flex items-start sm:items-center p-4 border-b border-gray-200 last:border-b-0 flex-col sm:flex-row">
              ...
              <Button variant="ghost" size="sm" onClick={() => removeFromCart(item.id)} className="ml-0 sm:ml-4 text-red-500 hover:text-red-700" aria-label={`Remove ${item.name} from cart`}>
