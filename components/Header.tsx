...
// import { SearchContext } from '../contexts/SearchContext'; // SearchContext no longer needed for header's primary action
...
export const Header: React.FC = () => {
  ...
  // const { setSearchTerm } = useContext(SearchContext) as SearchContextType; // Not needed if navigating directly
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (localSearchTerm.trim()) {
      // setSearchTerm(localSearchTerm); // No longer setting global context here for this primary action
      navigate(`${RoutePath.SearchResultsPage}?q=${encodeURIComponent(localSearchTerm.trim())}`);
      // setLocalSearchTerm(''); // Optionally clear after search
    }
  };
    ...
    <header className="bg-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
         ...
         {/* Search bar for mobile - shown below main nav */}
